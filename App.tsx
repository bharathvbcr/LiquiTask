...
const SidebarLoadingFallback: React.FC<{ isCollapsed: boolean }> = ({
	isCollapsed,
}) => (
...
const HeaderLoadingFallback: React.FC<{ sidebarOffset: number }> = ({
	sidebarOffset,
}) => (
	<div
...
const App: React.FC = () => {
	...
	const [showSubWorkspaceTasks, setShowSubWorkspaceTasks] =
	...
	const [creatingSubProjectFor, setCreatingSubProjectFor] = useState<
		string | undefined
	>(undefined);
	...
	>("default");
	const [commandUsageHistory, setCommandUsageHistory] = useState<
		Record<string, number>
	>({});
	// Refs for services
	const notificationServiceRef = useRef<NotificationServiceHandle | null>(null);
	const recurringTaskServiceRef = useRef<RecurringTaskServiceHandle | null>(
		null,
	);
	...
	const [priorities, setPriorities] =
		useState<PriorityDefinition[]>(defaultPriorities);
	...
	// Saved Views
	const { views, activeViewId, createView, applyView, deleteView } =
		useSavedViews();
	...
	const currentProjectTasks = useMemo(() => {
		if (showSubWorkspaceTasks) {
			const allProjectIds = [
				activeProjectId,
				...getAllSubWorkspaceIds(activeProjectId),
			];
	...
	const handleNaturalLanguageSearch = useCallback(
		async (query: string) => {
			try {
				...
				const { AIContext } = await import("./types");
				...
				if (result.filterGroup && result.filterGroup.rules.length > 0) {
					...
					addToast(
						"AI could not parse that query. Try standard search.",
						"info",
					);
				}
			} catch (e: any) {
				addToast(e.message || "Natural language search failed", "error");
			}
		},
		...
		try {
			...
			for (const suggestion of suggestions) {
				if (suggestion.confidence >= 0.7) {
					const task = currentProjectTasks.find(
			...
			if (applied > 0) {
				addToast(
					`AI applied ${applied} priority adjustment${applied > 1 ? "s" : ""}`,
					"success",
				);
			} else {
			...
			addToast(e.message || "AI prioritization failed", "error");
		} finally {
	...
	const commandActions = useMemo(() => {
		const baseActions: CommandAction[] = [
			...
			},
			{
				...
				category: "action",
				description:
					"Run batch AI operations: deduplicate, reprioritize, categorize",
		...
		return [...topProjectActions, ...baseActions].sort((a, b) =>
	...
	const debouncedSaveColumns = useMemo(
		() =>
			debounce(
				(cols: BoardColumn[]) => storageService.set(STORAGE_KEYS.COLUMNS, cols),
				500,
			),
		[],
	);
	const debouncedSaveProjects = useMemo(
		() =>
			debounce(
				(projs: Project[]) => storageService.set(STORAGE_KEYS.PROJECTS, projs),
				500,
			),
		[],
	);
	const debouncedSaveTasks = useMemo(
		() =>
			debounce(
				(tsks: Task[]) => storageService.set(STORAGE_KEYS.TASKS, tsks),
				500,
			),
		[],
	);
	...
	useEffect(() => {
		if (isLoaded)
			storageService.set(STORAGE_KEYS.ACTIVE_PROJECT, activeProjectId);
	...
	// --- Derived Data ---
	const activeProject: Project = projects.find(
	...
	const activeFilterCount = useMemo(() => {
		...
		if (activeFilterGroup.rules.length > 0)
			count += activeFilterGroup.rules.length;
		return count;
		...
		return currentProjectTasks
			.filter(
				(task) =>
					...
					(priorityId ? task.priority === priorityId : true),
			)
			.sort(
				(a, b) =>
					(a.order ?? a.createdAt.getTime()) -
					(b.order ?? b.createdAt.getTime()),
			);
	};
	...
	return (
		...
		>
			...
			<Suspense
				fallback={<SidebarLoadingFallback isCollapsed={isSidebarCollapsed} />}
			>
				<Sidebar
					projects={projects}
			...
			<main
				id="main-content"
				className="relative z-10 min-h-screen flex flex-col md:pl-[112px]"
			>
				<Suspense
					fallback={<HeaderLoadingFallback sidebarOffset={sidebarOffset} />}
				>
					<AppHeader
				...
				<div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
					<div className="flex flex-col items-center gap-4 rounded-2xl border border-white/10 bg-slate-900/90 px-8 py-6 shadow-2xl">
						...
						<p className="text-sm font-medium text-white">
							AI is analyzing task priorities...
						</p>
