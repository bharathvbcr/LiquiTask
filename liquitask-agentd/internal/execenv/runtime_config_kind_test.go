...
// withSlimBrief enables the `runtime_brief_slim` toggle for the duration of
// the test, then restores whatever value was wired before. Tests that
// exercise the slim path must call this; everything else gets the
// default-off behaviour and exercises the legacy path.
//
// The helper is NOT t.Parallel-safe because runtimeSlimBriefEnabled is a
// process-wide atomic.Bool. Tests that need the slim path stay serial.
// Hardly any slim test takes more than a few ms — serial is fine.
//
// Local-only fallback: upstream constructed a featureflag.StaticProvider +
// featureflag.Rule{Default: true} and wired it through featureflag.Service.
// That machinery (targeting rules, providers, decisions) doesn't exist in
// this sidecar — see the package-level note in runtime_config_flag.go — so
// this just flips the local bool directly via SetSlimBriefEnabled.
func withSlimBrief(t *testing.T) {
	t.Helper()
	saved := runtimeSlimBriefEnabled.Load()
	SetSlimBriefEnabled(true)
	t.Cleanup(func() { runtimeSlimBriefEnabled.Store(saved) })
}
...
func TestSlimFlagOffUsesLegacy(t *testing.T) {
	// Not parallel: reads runtimeSlimBriefEnabled without enabling slim, so
	// any test that races us by enabling slim would invalidate the
	// assertion.
	saved := runtimeSlimBriefEnabled.Load()
	t.Cleanup(func() { runtimeSlimBriefEnabled.Store(saved) })
	runtimeSlimBriefEnabled.Store(false)
