...
import (
	...
	"github.com/liquitask/liquitask-agentd/internal/skillbundle"
)
...
// skillRefFromBundle derives the content-addressed SkillRefData (hash, size,
// per-file digests) for a SkillData bundle.
//
// PORT NOTE: in upstream Multica this helper lives in server/internal/daemon/daemon.go
// (the big claim/runTask orchestrator), not in skill_cache.go. daemon.go itself is
// out of scope for this porting pass, but skill_cache_test.go depends on this
// function to build ref fixtures for its Load/Store round-trip tests. Rather than
// leave the test uncompilable, the helper is relocated here — it is a pure,
// server-independent function (only touches skillbundle.BuildManifest) and belongs
// naturally next to validateSkillBundle, which performs the same manifest
// computation in the opposite direction (verifying a ref against a bundle). When
// daemon.go's claim/runTask logic is ported in a later pass, drop the duplicate
// definition there and keep this one.
