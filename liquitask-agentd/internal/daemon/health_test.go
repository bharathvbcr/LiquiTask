...
// the health server binds and answers before local startup finishes, but it
// must report "starting" until d.ready is set, and only then "running".
// Otherwise a slow or failing startup would be misreported to callers (e.g.
// the Tauri frontend) as a fully started daemon.
