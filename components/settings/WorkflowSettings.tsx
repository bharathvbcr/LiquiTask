...
export const WorkflowSettings: React.FC<WorkflowSettingsProps> = ({
  localColumns, updateItem, setLocalColumns, deleteItem, saveAll
}) => {
  const addColumn = () => setLocalColumns([...localColumns, { id: `col-${Date.now()}`, title: 'New Column', color: '#64748b', wipLimit: 0 }]);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
      <div>
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={addColumn} className="text-xs flex items-center gap-1 text-red-400"><Plus size={14} /> Add Column</button>
        </div>
        <div className="space-y-2">
          {localColumns.map((col, idx) => (
            <div key={idx} className="flex items-center gap-2 p-2 bg-white/5 rounded-xl border border-white/5">
              <input type="color" value={col.color.startsWith('#') ? col.color : '#64748b'} onChange={(e) => updateItem(localColumns, idx, 'color', e.target.value, setLocalColumns)} className="w-6 h-6 rounded bg-transparent cursor-pointer" />
              <div className="flex-1 flex flex-col gap-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="number" min="0" value={col.wipLimit || ''} onChange={(e) => updateItem(localColumns, idx, 'wipLimit', parseInt(e.target.value) || 0, setLocalColumns)} className="bg-[#0a0e17] border border-white/10 rounded-md text-xs text-slate-400 p-0.5 w-12 text-center" placeholder="∞" />
                </div>
              </div>
              <button onClick={() => updateItem(localColumns, idx, 'isCompleted', !col.isCompleted, setLocalColumns)} className={`p-1.5 rounded-md ${col.isCompleted ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-600'}`}><CheckSquare size={14} /></button>
