...
describe('AiSettings Component', () => {
  ...
  it('shows Gemini fields by default and saves config', async () => {
    ...
    expect(storageService.set).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
      provider: 'gemini',
  ...
  it('shows Ollama model-test explanation', async () => {
    ...
    expect(
      screen.getByText(/Test Connection checks the Ollama service, confirms the model is installed, and asks that model for a real response\./i)
    ).toBeDefined();
  });
});
