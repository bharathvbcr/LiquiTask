---
name: services
description: "Skill for the Services area of LiquiTask. 273 symbols across 43 files."
---

# Services

273 symbols | 43 files | Cohesion: 78%

## When to Use

- Working with code in `src/`
- Understanding how useSearchHistory, useProjectController, loadData work
...
| File | Symbols |
|------|---------|
| `src/services/aiService.ts` | stripTaskData, AIProvider, refineTask, GeminiProvider, refineTask (+56) |
| `src/services/indexedDBService.ts` | isAvailable, saveTask, saveTasks, saveProject, saveColumns (+18) |
...
| `src/services/recurringTaskService.ts` | RecurringTaskService, initializeRecurringTaskService, getRecurringTaskService, calculateNextOccurrence, updateNextOccurrence (+5) |
...
- **`loadData`** (Function) — `src/hooks/useAppInitialization.ts:145`
...
| `_handleTestConnection` | Function | `components/settings/AiSettings.tsx` | 201 |
| `initializeRecurringTaskService` | Function | `src/services/recurringTaskService.ts` | 215 |
...
1. `gitnexus_context({name: "useSearchHistory"})` — see callers and callees
