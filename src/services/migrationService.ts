...
import type { BackupInfo, MigratableAppData, MigrationResult } from "../../types";
import { STORAGE_KEYS } from "../constants";
import { compareVersions, getCurrentDataVersion, getMigrationsFrom } from "../migrations";
...
export class MigrationService {
  ...
  private pruneOldBackups(): void {
    ...
    const sortedBackups = Array.from(this.backups.entries()).sort(
      (a, b) => b[1].info.timestamp.getTime() - a[1].info.timestamp.getTime(),
    );
