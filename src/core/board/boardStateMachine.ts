...
/**
 * The git-aligned four-stage board state machine.
 *
 * The board is a strict state machine mirroring the git lifecycle of a task:
 *
 *   Task ──────────▶ InProgress ──────────▶ Completed ──────────▶ Commit
 *   (backlog)        (worktree/branch       (work staged,          (merged;
 *                     active)                awaiting review)       terminal)
 *
 * Reverse edges: InProgress → Task (abort), Completed → InProgress (rework),
 * Completed → Task (park). Commit is terminal — reopening requires an explicit
 * `reopen` flag (user-confirmed) and lands back in Task.
 *
 * Custom (user-created) columns are treated as parking lanes: they can trade
 * cards with Task/InProgress freely, but entry into Completed and Commit is
 * always guarded by the canonical rules below.
 *
...
export interface TransitionContext {
  ...
  /**
   * True when the Completed→Commit move is being executed BY the transactional
   * merge pipeline (verify → commit → merge → prune). A card with unmerged
   * agent work may only enter Commit through that pipeline.
   */
  ...
  wipExceeded?: boolean;
  /** Explicit, user-confirmed reopen of a committed card (Commit → Task). */
  reopen?: boolean;
}
...
export interface TransitionVerdict {
   ...
   */
  requires?: "merge-pipeline" | "agent-run";
}
...
export function validateTransition(
  ...
  if (to === COLUMN_STATUS.COMMIT) {
    ...
    if (ctx.hasUnmergedWork && !ctx.viaMergePipeline) {
      // Legal, but only the transactional merge pipeline may execute it.
