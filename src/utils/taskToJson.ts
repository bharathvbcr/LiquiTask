...
export function taskToJson(task: Task, projectName?: string): string {
  const taskData = {
    ...
    timeEstimate:
      task.timeEstimate > 0 ? `${Math.round((task.timeEstimate / 60) * 10) / 10} hours` : undefined,
    timeEstimateMinutes: task.timeEstimate > 0 ? task.timeEstimate : undefined,
    timeSpent:
      task.timeSpent > 0 ? `${Math.round((task.timeSpent / 60) * 10) / 10} hours` : undefined,
    ...
    recurring: task.recurring
      ? {
          ...
          nextOccurrence: task.recurring.nextOccurrence
            ? task.recurring.nextOccurrence.toISOString()
            : undefined,
        }
      : undefined,
