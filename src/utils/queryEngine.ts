...
import type { FilterGroup, FilterRule } from "../types/queryTypes";
...
function evaluateRule(task: Task, rule: FilterRule): boolean {
  ...
  switch (rule.operator) {
    case "contains":
      return strTaskValue.includes(strRuleValue);
    case "not-contains":
      return !strTaskValue.includes(strRuleValue);
    case "equals":
      return strTaskValue === strRuleValue;
    case "not-equals":
      return strTaskValue !== strRuleValue;
    case "starts-with":
      return strTaskValue.startsWith(strRuleValue);
    case "ends-with":
      return strTaskValue.endsWith(strRuleValue);
    case "is-empty":
