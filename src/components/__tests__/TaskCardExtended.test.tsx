...
describe("TaskCard Extended", () => {
  ...
  it("renders correctly in expanded mode", () => {
    ...
    // Subtitle is no longer shown on the card face; the summary/custom fields are
    // rendered in the hover-reveal region (still present in the DOM).
  ...
  it("shows blocked status when task has blocked-by links", () => {
    ...
    // Blocked tasks are marked with a red left edge + a red indicator dot on the card.
    expect(screen.getByRole("article")).toHaveClass("border-l-red-500/50");
  });
});
