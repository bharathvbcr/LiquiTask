...
interface ProjectModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (name: string, icon: string, parentId?: string, workspacePaths?: string[]) => void;
  projects?: Project[];
    ...
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="space-y-2">
          ...
          {workingDir ? (
            <div className="flex items-center gap-2 group px-3 py-2 bg-[#05080f] border border-white/5 rounded-xl">
              <FolderOpen size={13} className="text-red-400/70 shrink-0" />
              <span className="flex-1 text-[12px] text-slate-400 font-mono truncate" title={workingDir}>
                {workingDir}
              </span>
              <button
                type="button"
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 px-4 py-3 rounded-xl bg-[#05080f] border border-dashed border-white/10 text-[12px] text-slate-500 hover:border-red-500/30 hover:text-red-400 transition-colors"
            >
              <FolderOpen size={13} />
              Select Directory
            </button>
