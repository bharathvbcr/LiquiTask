 ...
 */
import { pointerWithin, closestCorners, CollisionDetection } from '@dnd-kit/core';
...
export const ProjectBoard: React.FC<ProjectBoardProps> = (props) => {
    const {
        columns = [], priorities = [], tasks = [], allTasks = [], boardGrouping,
        onUpdateColumns, onMoveTask, onEditTask, onUpdateTask, onDeleteTask,
        onMoveBlocked, canMoveTask, getTasksByContext, isCompact = false,
        onCopyTask, projectName, projects = [], onMoveToWorkspace, addToast,
    } = props;
    ...
    const emitMoveBlocked = useCallback((message: string) => {
        onMoveBlocked ? onMoveBlocked(message) : showToast(message, 'error');
    }, [onMoveBlocked, showToast]);

    const { activeDrag, highlightedZone, sensors, handleDragStart, handleDragOver, handleDragEnd, handleDragCancel } = useBoardDnDController({
        columns, tasks, boardGrouping, onUpdateColumns, onMoveTask, canMoveTask, getTasksByContext, showToast
    });

    const { focusedColumnIndex, focusedTaskId, handlers: keyboardHandlers } = useBoardKeyboardNav({
        columns, tasks, onMoveTask, canMoveTask, onEditTask, onDeleteTask, getTasksByContext, boardGrouping, onMoveBlocked: emitMoveBlocked, isEnabled: true,
    });
    ...
    const measuringConfig = { droppable: { strategy: MeasuringStrategy.Always, frequency: 100 } };
    ...
    const commonProps = {
        sensors, collisionDetection: kanbanCollisionDetection, measuringConfig, dropAnimation,
        boardRef, columns, priorities, tasks, allTasks, highlightedZone, focusedTaskId, activeTask,
        isCompact, onDragStart: handleDragStart, onDragOver: handleDragOver, onDragEnd: handleDragEnd,
        onDragCancel: handleDragCancel, onMoveTask, onEditTask, onUpdateTask, onDeleteTask,
        getTasksByContext, onCopyTask, projectName, projects, onMoveToWorkspace
    };
    ...
    return (
        <Suspense fallback={<BoardLoadingFallback />}>
            {boardGrouping === 'priority' ? (
                ...
                <StandardBoardView {...commonProps} activeColumn={activeColumn} columnIds={columns.map(c => c.id)} focusedColumnIndex={focusedColumnIndex} />
            )}
        </Suspense>
    );
};
