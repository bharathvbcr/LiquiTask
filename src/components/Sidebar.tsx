...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
      <div className="flex flex-col gap-0.5">
        ...
        <div
          ...
          className={`
                group relative px-3 py-2.5 rounded-xl cursor-pointer transition-[background-color,color,transform] duration-200
                flex items-center overflow-visible border border-transparent
        ...
        >
          ...
          <div
            className={`flex items-center relative z-10 overflow-hidden ${isEffectivelyCollapsed ? "justify-center" : "w-full gap-3"}`}
          >
            {/* Depth indent + expand chevron — only rendered when needed so leaf
                top-level project icons line up with the Dashboard/Archive icons. */}
            {(depth > 0 || hasVisibleChildren) && (
              <div
  ...
  return (
    ...
    >
      <div
        className="pointer-events-auto flex h-full w-80 flex-col overflow-hidden rounded-[28px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform liquid-glass liquid-glass--soft border border-white/10"
        style={{
      ...
      >
        ...
        >
          <div className={`flex items-center ${isEffectivelyCollapsed ? "gap-0" : "gap-3"}`}>
            <img
              src={logo}
              alt="LiquiTask"
              className="h-8 w-8 shrink-0 object-contain opacity-100 drop-shadow-[0_4px_12px_rgba(255,31,31,0.4)] transition-[width,height,opacity] duration-300 ease-out"
            />
            <div
        ...
        >
          ...
          <div
            className={`h-px bg-white/5 mx-2 transition-all duration-300 ${isEffectivelyCollapsed ? "my-0 max-h-0 opacity-0" : "my-1 opacity-100"}`}
            aria-hidden="true"
          />
        ...
        {/* Footer */}
        <div
          className={`mt-auto shrink-0 ${isEffectivelyCollapsed ? "p-3" : "border-t border-white/5 bg-white/[0.02] p-4"}`}
        >
          <Tooltip
            content="Settings"
            position={isEffectivelyCollapsed ? "right" : "top"}
          ...
          >
            <button
              ...
              onMouseLeave={() => setHoveredItem(null)}
              className={`
                group flex items-center rounded-xl hover:bg-white/5 cursor-pointer
                ...
                ${isEffectivelyCollapsed ? "mx-auto h-11 w-11 justify-center" : "w-full gap-3 px-3 py-3"}
              `}
            >
