...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  const filteredActions = useMemo(() => {
    const quickCreateResult =
      query.trim() && onCreateTask && parsedTask?.title
        ? [
            {
    ...
    return [...quickCreateResult, ...sortedCandidates.map((candidate) => candidate.action)];
  }, [actions, commandUsageHistory, onCreateTask, parsedTask, query]);

  const groupedActions = useMemo(() => groupByCategory(filteredActions), [filteredActions]);
  ...
  const renderTaskPreview = () => {
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-1">
        {parsedTask.priority && (
          <span
            className={`flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded uppercase font-bold ${
        ...
        {parsedTask.tags.map((tag) => (
          <span
            key={tag}
            className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-slate-300"
          >
            <Tag size={10} /> {tag}
          </span>
        ))}
          ...
          <div ref={listRef} className="max-h-[50vh] overflow-y-auto custom-scrollbar">
            {filteredActions.length === 0 ? (
              ...
              Object.entries(groupedActions).map(
                ([category, categoryActions]: [string, CommandAction[]]) => {
                    ...
                    <div key={category} className="py-2">
                      ...
                      {categoryActions.map((action) => {
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`shrink-0 ${isSelected ? "text-red-400" : "text-slate-500"}`}
                            >
