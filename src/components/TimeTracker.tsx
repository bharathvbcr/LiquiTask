...
export const TimeTracker: React.FC<TimeTrackerProps> = ({
  ...
  const progressPercent =
    estimateSeconds > 0 ? Math.min((seconds / estimateSeconds) * 100, 100) : 0;
  ...
  return (
    <div className="bg-black/30 rounded-xl p-4 border border-white/5">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {task.timeEstimate > 0 && (
          <div className="text-xs text-slate-500">Est: {formatMinutes(task.timeEstimate)}</div>
        )}
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 gap-4 text-center">
        ...
        <div>
          ...
          <div
            className={`text-sm font-bold ${isOverEstimate ? "text-red-400" : "text-slate-300"}`}
          >
            {estimateSeconds > 0
              ? isOverEstimate
