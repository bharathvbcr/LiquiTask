...
import { Task, BoardColumn, PriorityDefinition, ToastType, ActivityType, ActivityItem, Project } from '../../types';
...
type ActivityServiceLike = {
  createActivity: (type: ActivityType, details: string, field?: string, oldValue?: unknown, newValue?: unknown) => ActivityItem;
...
export const useTaskController = ({
  ...
  const handleUpdateTaskDueDate = useCallback((taskId: string, newDate: Date) => {
    ...
    const updatedTask = activityServiceRef.current?.logChange(task, updates) || { ...task, ...updates };
  ...
  const handleCreateOrUpdateTask = useCallback((taskData: Partial<Task>, editingTask: Task | null) => {
    if (editingTask) {
      const previousTask = tasks.find(t => t.id === editingTask.id);
      const updates = { ...taskData, updatedAt: new Date() };
      let updatedTask = activityServiceRef.current?.logChange(editingTask, updates) || { ...editingTask, ...updates };
      ...
      if (updatedTask.recurring?.enabled && recurringService && !updatedTask.recurring.nextOccurrence) {
    ...
    const newTask: Task = {
      ...
      activity: activityServiceRef.current ? [activityServiceRef.current.createActivity('create', 'Task created')] : [],
  ...
  }, [tasks, activeProjectId, columns, pushUndo, addToast, automationServiceRef, activityServiceRef, recurringTaskServiceRef, searchIndexServiceRef]);

  const handleBulkCreateTasks = useCallback((newTasksData: Partial<Task>[]) => {
    const now = new Date();
  ...
  const moveTask = useCallback((taskId: string, newStatus: string, newPriority?: string, newOrder?: number) => {
    const task = tasks.find(t => t.id === taskId);
    if (!task) {
      addToast('Task not found', 'error');
      return;
    ...
    if (newStatus !== task.status && activityServiceRef.current) {
      activity.push(activityServiceRef.current.createActivity('move', `Moved to ${columns.find(c => c.id === newStatus)?.title}`, 'status', task.status, newStatus));
    }
    if (newPriority && newPriority !== task.priority && activityServiceRef.current) {
      activity.push(activityServiceRef.current.createActivity('update', `Priority changed to ${newPriority}`, 'priority', task.priority, newPriority));
    }
  ...
  }, [tasks, columns, addToast, pushUndo, automationServiceRef, activityServiceRef, recurringTaskServiceRef, searchIndexServiceRef]);

  const canMoveTask = useCallback((taskId: string, newStatus: string, _newPriority?: string) => {
    const task = tasks.find(t => t.id === taskId);
    ...
    if (newStatus !== columns[0].id) {
      ...
      for (const link of blockedLinks) {
        ...
        if (blocker) {
          const blockerCol = columns.find(c => c.id === blocker.status);
          if (!blockerCol?.isCompleted && blocker.status !== COLUMN_STATUS.DELIVERED) {
            return { allowed: false, reason: `Cannot start: Blocked by task ${blocker.jobId}` };
          }
        }
      }
    }
    ...
    if (newStatus !== task.status && targetColumn.wipLimit && targetColumn.wipLimit > 0) {
      const tasksInColumn = tasks.filter(t => t.status === newStatus && t.id !== taskId);
      if (tasksInColumn.length >= targetColumn.wipLimit) {
        return { allowed: false, reason: `Column "${targetColumn.title}" has reached its WIP limit` };
      }
    }
