...
interface UseBoardDnDControllerProps {
    ...
    canMoveTask?: (taskId: string, newStatus: string, newPriority?: string) => { allowed: boolean; reason?: string };
...
export const useBoardDnDController = ({
    ...
    const resolveDropTarget = useCallback((
        ...
        if (boardGrouping === 'priority' && overId.includes('::')) {
            ...
            if (columns.some(c => c.id === statusId)) return { status: statusId, priority: priorityId };
        }
    ...
    const handleColumnReorder = useCallback((activeId: string, overId: string) => {
        ...
        if (oldIndex !== -1 && newIndex !== -1) onUpdateColumns(arrayMove(columns, oldIndex, newIndex));
    }, [columns, onUpdateColumns]);

    const handleTaskDrop = useCallback((task: Task, overId: string) => {
        ...
        const moveValidation = canMoveTask ? canMoveTask(task.id, status, priority) : { allowed: true };
        if (!moveValidation.allowed) {
            showToast(moveValidation.reason || 'Cannot move task', 'error');
            return;
        }

        if (status !== task.status || (priority !== undefined && priority !== task.priority) || order !== undefined) {
