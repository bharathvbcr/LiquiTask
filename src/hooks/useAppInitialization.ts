...
import type { ActivityType, BoardColumn, CustomFieldDefinition, GroupingOption, PriorityDefinition, Project, ProjectType, Task, TaskTemplate, ToastType } from '../../types';
...
type NotificationTask = { id: string; title: string; dueDate?: Date; status?: string; completedAt?: Date };

type NotificationServiceLike = {
  requestPermission: () => Promise<boolean>;
  show: (options: { title: string; body: string; icon?: string; tag?: string; silent?: boolean; onClick?: () => void }) => void;
  startPeriodicCheck: (getTasks: () => NotificationTask[], intervalMs?: number) => void;
  stopPeriodicCheck: () => void;
};

type ActivityServiceLike = {
  createActivity: (type: ActivityType, details: string, field?: string, oldValue?: unknown, newValue?: unknown) => unknown;
...
export const useAppInitialization = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (data.columns) {
        ...
        if (indexedDBService.isAvailable()) indexedDBService.saveColumns(data.columns).catch(console.error);
      }
      if (data.projectTypes) setProjectTypes(data.projectTypes);
      if (data.priorities) {
        setPriorities(data.priorities);
        if (indexedDBService.isAvailable()) indexedDBService.savePriorities(data.priorities).catch(console.error);
      }
      if (data.customFields) {
        setCustomFields(data.customFields);
        if (indexedDBService.isAvailable()) indexedDBService.saveCustomFields(data.customFields).catch(console.error);
      }
      if (data.projects) {
        setProjects(data.projects);
        if (indexedDBService.isAvailable()) Promise.all(data.projects.map(p => indexedDBService.saveProject(p))).catch(console.error);
      }
      if (data.tasks) {
        setTasks(data.tasks);
        ...
        if (indexedDBService.isAvailable()) indexedDBService.saveTasks(data.tasks).catch(console.error);
      }
      ...
      import('../services/templateService').then(({ templateService }) => {
        ...
        templateService.loadTemplates(storageService.get('liquitask-templates', []) as TaskTemplate[]);
      });
  ...
  useEffect(() => {
    ...
    import('../services/recurringTaskService').then(({ initializeRecurringTaskService, getRecurringTaskService }) => {
      if (!isActive) return;
