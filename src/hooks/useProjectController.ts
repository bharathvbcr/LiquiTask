...
import { Project, ProjectType, Task, ToastType } from '../../types';
...
export const useProjectController = ({
  ...
  const handleCreateProject = useCallback((name: string, icon: string, parentId?: string) => {
  ...
  const handleEditProject = useCallback((projectId: string, newName: string, newIcon: string) => {
    setProjects(prev => prev.map(p => {
