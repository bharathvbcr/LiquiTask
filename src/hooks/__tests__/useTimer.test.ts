...
import { formatMinutes, secondsToMinutes, useTimer } from "../useTimer";
...
describe("useTimer", () => {
  ...
  describe("auto-save", () => {
    it("should call onAutoSave at specified interval", () => {
      ...
      const { result } = renderHook(() => useTimer({ autoSaveInterval: 2, onAutoSave }));
    ...
    it("should not auto-save when paused", () => {
      const onAutoSave = vi.fn();
      const { result } = renderHook(() => useTimer({ autoSaveInterval: 1, onAutoSave }));
