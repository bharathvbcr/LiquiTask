...
describe('useTaskController', () => {
  ...
  const mockRecurringTaskService = { calculateNextOccurrence: vi.fn(), updateNextOccurrence: vi.fn() };
  const mockSearchIndexService = { updateTask: vi.fn(), removeTask: vi.fn() };

  const initialTasks: Task[] = [
    { id: 'task-1', title: 'Task 1', status: 'Pending', projectId: 'p1', priority: 'medium', createdAt: new Date(), updatedAt: new Date(), jobId: 'TSK-1', subtasks: [], attachments: [], tags: [] }
  ];
  ...
  const props = {
    initialTasks,
    columns,
    projects: [{ id: 'p1', name: 'Project 1' }, { id: 'p2', name: 'Project 2' }],
    priorities,
    activeProjectId: 'p1',
