...
describe('useAppInitialization', () => {
  ...
  it('registers recurring callbacks with functional task updates', async () => {
    ...
    const newTask = makeTask({ id: 'new-task', title: 'Generated recurring task' });

    await act(async () => {
      recurringOptions!.onCreateTask(newTask);
      recurringOptions!.onUpdateTask('existing-task', { title: 'Updated title' });
    });
    ...
    const withNewerState = createUpdater([
      ...
      makeTask({ id: 'later-task', title: 'Created after effect registration' }),
    ]);
    expect(withNewerState.map(task => task.id)).toEqual(['existing-task', 'later-task', 'new-task']);
    ...
    expect(pushUndo).toHaveBeenCalledWith({ type: 'task-create', taskId: 'new-task' });
    expect(addToast).toHaveBeenCalledWith('Recurring task "Generated recurring task" created', 'info');
  });
});
