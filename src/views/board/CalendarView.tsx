...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const [aiSuggestedDates, setAiSuggestedDates] = useState<Record<string, string[]>>({});
  ...
  const handleAiSchedule = async () => {
    ...
    try {
      ...
      for (const task of tasksWithoutDueDate.slice(0, 10)) {
        const result = await aiService.suggestSchedule(task, tasks, context);
        if (result.suggestedDueDate) {
          const dateKey = normalizeDate(result.suggestedDueDate).toDateString();
          if (!suggestions[dateKey]) suggestions[dateKey] = [];
          suggestions[dateKey].push(task.id);
        }
      }
      setAiSuggestedDates(suggestions);
    } catch (e) {
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col">
        ...
        {/* Empty state banner: nothing scheduled in the visible range */}
        {!displayDays.some((date) => (tasksByDate.get(getDateKey(date)) || []).length > 0) && (
          <div className="flex items-center justify-center gap-2 mb-3 px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-400 animate-in fade-in">
            <CalendarIcon size={16} className="text-slate-500" />
            {tasks.length === 0
              ? "No tasks yet — click any day to schedule one."
              : `Nothing scheduled this ${viewMode}. Drag tasks here or click a day to add one.`}
          </div>
        )}
                      ...
                      {aiSuggestedDates[dateKey].map((taskId) => {
                        ...
                        return (
                          <button
                            ...
                            title={`Set due date to ${date.toLocaleDateString()} - ${task.title}`}
                          >
                            {task.title}
                          </button>
                        );
      ...
      {createPortal(
        <DragOverlay dropAnimation={dropAnimation}>
          {activeTask && (
            <div
              className="px-2 py-1 rounded-md text-[10px] font-medium shadow-2xl scale-105 rotate-2"
              style={{
